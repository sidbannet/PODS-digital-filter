...
def contourf(MinV, MaxV, fig, x, y, z, levels, x_axis, y_axis, c_bar, filename, figsize=(8, 6)):
    # plt.rc('text', usetex=True)
    # plt.rc('font', family='serif')
...
def arrow(fig, y, t, dx, dy, y_axis, x_axis, filename):
    ...
    for i in range(0, len(dy)):
        f = plt.arrow(t[i], y[i], dy[i], dx[i], length_includes_head=True, head_width=0.001, head_length=0.001,
                      width=0.0001, fc='k', ec='k')
    plt.xlabel('$' + x_axis + '$')
    plt.ylabel('$' + y_axis + '$')
    plt.axis('equal')
    if (filename != ' '):
...
def arcarrow(fig, y, t, dx, dy, y_axis, x_axis, filename):
    ...
    for i in range(0, len(dy)):
        ...
        # print arc
        ax.add_patch(l[i])
        f = plt.arrow(t2 + 0.001, x2, -0.0001 * x2, 0.0001 * t2, length_includes_head=False, head_width=0.001,
                      head_length=0.001, width=0.0001, fc='k', ec='k')
    # ax.plot([0,1],[0,1],'k')
    plt.xlabel('$' + x_axis + '$')
    plt.ylabel('$' + y_axis + '$')
